...
case class SSHBasedStrategy(topology: Topology,
    ...
    topology.nodes.foreach { instance =>
      ...
      keypairOption.map(keyPair =>
        keyPair.filePath.map(keyFilePath => {
          ...
          val newCloudInstance = collectInstanceDetails(topology.site, cloudInstance, userName.get, keyFilePath, keyPair.passPhrase)
          logger.info(s"Establishing service to collect instance details ${newCloudInstance}")
        }
        )
      )
    }
  ...
  def collectInstanceDetails(site: Site1, cloudInstance: CloudInstance, userName: String, keyLocation: String, passPharse: Option[String]): CloudInstance = {
      ...
      if (osInfo.isDefined && osInfo.get.toLowerCase().contains("ubuntu")) {
        ...
        if (connectionInfo.isDefined) {
          ...
          if (processList.isDefined) {
           val (cloudInstanceResult ,siteResult) = resolveProcesses(site, cloudInstanceNew3.asInstanceOf[CloudInstance], connectionOption, processList.get, sshSession)
            //TODO: Return cloudInstanceResult and siteResult
            logger.info(s"Site ${siteResult}")
            logger.info(s"CloudInstance ${cloudInstanceResult}")
          }else{
  ...
  private def resolveProcesses(site1: Site1, cloudInstance: CloudInstance, serverIp: String, processList: String, sshSession: SSHSession) : (CloudInstance,Site1) = {
    /*
    ...
    // Remove process tag in CloudInstance
    val cloudInstance1 = cloudInstance.copy(tags = cloudInstance.tags.filter(tag => !tag._1.equals(PROCESS_TAG))).asInstanceOf[CloudInstance]
    // Find the process map
    // Iterate and add process details in CloudInstance
   val (cloudInstanceResult , siteResult) = findProcessMap(processList).foldLeft((cloudInstance1 -> site1))((acc, process)  => {
      ...
      // Update processInfo and tags in CloudInstance
      val cloudInstance2: CloudInstance = cloudInstance.copy(processes = cloudInstance.processes +
        new ProcessInfo(pid = pId.toInt
          , parentPid = pId.toInt
          , name = processName
        ...
        , tags = (PROCESS_TAG -> processName) :: cloudInstance1.tags).asInstanceOf[CloudInstance]

      softwareAssociatedWithProcessOpt.map(softwareAssociatedWithProcess => {
        val cloudInstance3 = setRole(cloudInstance2,softwareAssociatedWithProcess)
        if(softwareAssociatedWithProcess.discoverApplications){
          ...
          val cloudInstanceFinal = apps.foldLeft(cloudInstance3)((accCloudInstance, app) => {
            accCloudInstance.copy(tags = (accCloudInstance.tags :+ (APPLICATION_TAG_KEY,app))).asInstanceOf[CloudInstance]
          })
  ...
  private def setRole(instance: CloudInstance, softwareAssociatedWithProcess: Software): CloudInstance = {
    val roleOption = InstanceRole.hasSoftware(softwareAssociatedWithProcess)
    val tagOption = instance.tags.find(tag => tag._2.equals(ROLES_TAG_KEY))
    val role = roleOption.map(r => {
      tagOption.map(tag => {
        tag.copy(_2 = r.name)
      }).getOrElse(ROLES_TAG_KEY -> r.name)
    })
    role.map(role => instance.copy(tags = role :: instance.tags))
      .getOrElse(instance).asInstanceOf[CloudInstance]
  }
