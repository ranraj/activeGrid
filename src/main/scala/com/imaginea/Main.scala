...
object Main extends App {
  ...
  implicit val applicationTire = jsonFormat(ApplicationTier.apply, "id", "name", "description", "instances"/*, "apmServer"*/)
  implicit val applicationTires = jsonFormat(ApplicationTiers.apply, "id", "name", "descritption", "instances"/*, "aPMServerDetails"*/)
  ...
  val discoveryRoutes = pathPrefix("discover") {
        ...
        put {
          entity(as[Site1]) { site =>
            val buildSite = Future {
              ...
              val site1 = Site1(Some(savedSite.getId), site.siteName, computedResult._1, computedResult._2, site.filters, computedResult._3, computedResult._4,List.empty[Application], List())
          ...
          val filteredSite = Future {
            ...
            mayBeSite match {
              case Some(site) =>
                ...
                Some(Site1(site.id, site.siteName, listOfFilteredInstances, site.reservedInstanceDetails, site.filters, site.loadBalancers, site.scalingGroups,List.empty[Application], site.groupsList))
              case None =>
                logger.warn(s"Failed while doing fromNeo4jGraph of Site for siteId : $siteId")
                None
            }
    ...
    path("sites" / LongNumber / "topology") { siteId =>
      post {
        ...
        siteOption.foreach{site =>
          //site.instances.map(instance => instance)
          ...
          val getSoftwares = Future {
            ...
            val shhBaseStatergy = new SSHBasedStrategy(topology,softwares,true)
          }
